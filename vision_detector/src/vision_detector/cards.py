"""Shared card-set and deck-manifest domain types."""
...
class CardContractModel(BaseModel):
    """Immutable, closed JSON data for the shared card configuration."""
...
class DeckManifest(CardContractModel):
    """One explicit Doppelkopf deck variant."""
    ...
    def validate_contents(self) -> DeckManifest:
        ...
        calculated_count = sum(card.copies for card in self.cards)
        if self.physical_card_count != calculated_count:
...
def load_deck_manifest(variant: str, path: Path | None = None) -> DeckManifest:
    """Load one canonical deck manifest by variant."""

    manifest_path = path or default_deck_manifest_path(variant)
    return _load_json_model(manifest_path, DeckManifest)
