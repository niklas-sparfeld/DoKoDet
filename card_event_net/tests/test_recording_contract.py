...
from cardevent.recording_contract import (
    DevicePredictions,
...
def test_shared_malformed_variants_are_rejected(name, change, expected_message) -> None:
    manifest_bytes, predictions_bytes, video_bytes, manifest, predictions = load_fixture()
    manifest = copy.deepcopy(manifest)
    predictions = copy.deepcopy(predictions)
    change(manifest, predictions)

    with pytest.raises(RecordingContractError, match=expected_message) as error:
        validate_recording_bundle(
