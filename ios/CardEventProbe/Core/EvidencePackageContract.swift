...
/// Repository intake metadata that travels with each accepted evidence package.
public struct EvidencePackageRepositoryMetadata: Codable, Equatable, Sendable {
    public let packageRecord: RepositoryEvidencePackageRecord
    public let taskEnrollment: RepositoryTaskEnrollmentDocument
    public let lineage: RepositoryEvidencePackageLineage

    public init(
        packageRecord: RepositoryEvidencePackageRecord,
        taskEnrollment: RepositoryTaskEnrollmentDocument,
        lineage: RepositoryEvidencePackageLineage
    ) throws {
        guard packageRecord.packageID == lineage.packageID,
              packageRecord.sourceAssetID == taskEnrollment.sourceAssetID else {
            throw EvidencePackageError.invalidRepositoryMetadata
        }
    ...
    public static func standard(for manifest: EvidencePackageManifest) throws -> Self {
        let packageID = manifest.packageID.uuidString.lowercased()
        let sourceAssetID = "source-evidence-\(packageID)"
        let createdAtUTC = "1970-01-01T00:00:00Z"
        let enrollments = try RepositoryDataTask.allCases.map { task in
            try RepositoryTaskEnrollment(
                taskEnrollmentID: "enrollment-\(packageID)-\(task.rawValue)",
                task: task,
                disposition: .selected,
                operatorName: "ios-client",
        ...
        return try Self(
            packageRecord: RepositoryEvidencePackageRecord(
                ...
                notes: "Generated by the CardEventProbe producer."
            ),
            taskEnrollment: RepositoryTaskEnrollmentDocument(
                sourceAssetID: sourceAssetID,
            ...
            lineage: RepositoryEvidencePackageLineage(
                packageID: packageID,
                parentSourceAssetID: nil,
                parentRecordingID: nil,
                parentVideoID: nil,
                sessionID: manifest.session.sessionID.uuidString.lowercased()
            )
        )
    }

    public func encodedDocuments() throws -> (packageRecord: Data, taskEnrollment: Data, lineage: Data) {
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.sortedKeys]
        return (
            try encoder.encode(packageRecord),
            try encoder.encode(taskEnrollment),
            try encoder.encode(lineage)
        )
    }
}
...
public enum EvidencePackageError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .invalidRepositoryMetadata:
            return "The evidence package repository metadata has different identities."
        }
    }
}
...
public struct EvidencePackage: Sendable {
    ...
    public let repositoryMetadata: EvidencePackageRepositoryMetadata?

    public init(
        manifest: EvidencePackageManifest,
        frames: [PackagedEvidenceFrame],
        videoSnippet: PackagedEvidenceVideo? = nil,
        repositoryMetadata: EvidencePackageRepositoryMetadata? = nil
    ) throws {
        ...
        for frame in frames {
            ...
            guard frame.manifest.byteLength == frame.jpegData.count,
                  ...
                  evidencePackageSHA256Hex(frame.jpegData) == frame.manifest.sha256 else {
                throw EvidencePackageError.frameDataMismatch(frame.manifest.partName)
            }
        ...
        if let declaredVideo = manifest.videoSnippet, declaredVideo.captureComplete {
            guard let partName = declaredVideo.partName,
                  ...
                  evidencePackageSHA256Hex(videoSnippet.mp4Data) == declaredVideo.sha256 else {
...
internal func evidencePackageSHA256Hex(_ data: Data) -> String {
