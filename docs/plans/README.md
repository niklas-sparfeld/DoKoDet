...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0080 — Agent-scale visible-card review modules](3-in-progress/0080-Agent_Scale_Visible_Card_Review_Modules.md) | Completed 0059 web workspace module boundaries and completed 0074 unified visible-card review workbench | M0–M3 complete: frozen public contracts and characterization coverage; extracted workbench controls, surface interaction, and layers plus editor data, geometry, formatting, and URL helpers with focused tests. M4–M5 not started. The 10 existing editor test failures remain recorded as baseline. |
