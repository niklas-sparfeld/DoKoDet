...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0080 — Agent-scale visible-card review modules](3-in-progress/0080-Agent_Scale_Visible_Card_Review_Modules.md) | Completed 0059 web workspace module boundaries and completed 0074 unified visible-card review workbench | M0–M4 complete. M4 moves reference hydration, generated/proposal loading and polling, calibration refinement, ordered commands, retries, conflict recovery, reference rebases, completion, and local draft synchronization into the controller. Six controller tests pass. M5 not started. The 10 existing editor test failures remain recorded as baseline. |
