...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0065 — Visible-card ignore regions](3-in-progress/0065-Visible_Card_Ignore_Regions.md) | 0048, 0049, and 0062 complete | M0 is complete: the strict `ignored_regions` contract, typed API schema, generated client, lineage, and raster/overlap rules are frozen. M1–M3 remain. |
...
2. **Continue 0065 with M1 for untidy visible-card stacks.** Add reviewed ignore regions before
   affected `IMG_0661` frames enter a visible-card dataset or the 0051 identity freeze. Preserve
   Gemini proposals as generated evidence. Do not create an `untidy_stack` model class.
