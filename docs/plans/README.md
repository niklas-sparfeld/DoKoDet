...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0080 — Agent-scale visible-card review modules](3-in-progress/0080-Agent_Scale_Visible_Card_Review_Modules.md) | Completed 0059 web workspace module boundaries and completed 0074 unified visible-card review workbench | M0–M2 complete: frozen public contracts and characterization coverage; extracted workbench controls, proposal presentation, surface interaction, layers, and geometry with focused tests. M3–M5 not started. The 10 existing editor test failures remain recorded as baseline. |
