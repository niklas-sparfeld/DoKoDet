...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0072 — Pose-based visible-card review](3-in-progress/0072-Pose_Based_Visible_Card_Review.md) | 0048 and 0049 complete | M0–M4 complete: shared calibrated card-plane geometry and versioned scene contracts are frozen; M1 mines deterministic isolated-card candidates and stores immutable calibration revisions; M2 initializes fixed-size poses and deterministic stacking order; M3 adds the synchronized source-frame and rectified virtual-table editor; M4 derives and validates reviewed visible regions through the maintained-reference command queue and gates identity and dataset use on the validated scene view. M5 not started. |
