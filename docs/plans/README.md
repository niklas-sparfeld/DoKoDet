...
| [0046 — Filesystem storage engine](3-in-progress/0046-Filesystem_Storage_Engine.md) | None | M0 complete: filesystem ownership inventory and tested path-safe atomic storage primitives; M1 complete: recording-bundle reads, catalog refresh, upload replay/conflict handling, and publication without the SQL index; M2 complete: evidence-package and table-observation reads, pending analyzer selection, uniqueness, restart, and staged publication without SQL metadata; M3–M4 remain. |
