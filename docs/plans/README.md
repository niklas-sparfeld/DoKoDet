...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0068 — Reviewed RF-DETR local visible-card detector](3-in-progress/0068-Reviewed_RF_DETR_Local_Visible_Card_Detector.md) | 0037, 0048, 0049, 0065, and 0067 complete | M0–M2 implementation is complete: the reviewed-reference audit freezes source-group-safe partitions, the shared materializer emits deterministic train, valid, and sealed-test views, and the RF-DETR smoke and candidate paths verify digests, record device facts, retain resumable failures, and reuse completed bundles. Live training awaits the missing 24-recording snapshot and frozen M0 manifest. |
...
1. **Continue 0068 with M3 for the reviewed RF-DETR detector.** Run validation and sealed-test
   evidence only after the frozen M0 snapshot is available and the M2 candidate is retained.
   Reuse the verified M1 view and keep the sealed-test partition unread during training.
