...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0063 — Current CardEventNet training campaign](3-in-progress/0063-Current_CardEventNet_Training_Campaign.md) | 0020, 0028, 0048, and 0049 complete; 0062 M0 and M1 complete | M0–M5 complete. The first campaign retained a loadable checkpoint but failed its gates. The interval pass is complete for future-eligible recordings. M6 publishes the audit and marks `IMG_2777` through `IMG_2781` as repository-wide legacy-device diagnostic only. Later milestones freeze 27 train, six validation, and five test recordings, isolate the interval-data change, and make all long commands operator-run. |
...
1. **Continue 0063 with M6.** Audit the completed interval pass and publish the repository-wide
   diagnostic-only exclusion for `IMG_2777` through `IMG_2781`. Keep these old-phone recordings out
   of future datasets, negative mining, gates, and promotion decisions. The operator runs all long
