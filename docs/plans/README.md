...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0063 — Current CardEventNet training campaign](3-in-progress/0063-Current_CardEventNet_Training_Campaign.md) | 0020, 0028, 0048, and 0049 complete; 0062 M0 and M1 complete | M0–M13 and the operator-run hard-negative ablation are complete. M10 publishes 70 timing-review items in 47 deterministic regions for six recordings. M11 reconciles the prose notes into immutable item decisions, freezes successor dataset `cardeventnet-interval-dataset-00a59b5fcd210d23c569`, and measures three validation-only decoder responses without selecting one. M12 prepares and the operator completes one interval-aware endpoint response with a 125 ms stable-end target. M13 compares it with M9 on the same successor validation references and records `human_review_required`: 54 stable-end matches, 35 duplicates, and 86.77% event-presence recall fail the declared gates. No candidate lock or sealed-test read exists. |
