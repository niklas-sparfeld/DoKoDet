...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0070 — Synthetic visible-region training data](3-in-progress/0070-Synthetic_Visible_Region_Training_Data.md) | 0068 frozen training inputs plus explicitly reviewed training-only card sources; face-down cutout required before face-down or mixed-side rendering | M0–M3 complete with a declared gap; M4 preflight is ready; M5 now audits all 24 recordings and emits 38 train-only one/two/three-card scenes across 15 training recordings. Validation and sealed-test recordings are discovery-only. Full RF-DETR training remains operator-started and has not run. Face-down and mixed-side buckets remain omitted until a reviewed face-down cutout is available. |
