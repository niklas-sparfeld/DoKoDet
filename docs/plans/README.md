...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0075 — Robust automatic table-plane calibration](3-in-progress/0075-Robust_Automatic_Table_Plane_Calibration.md) | Completed 0072 and 0073 | M0–M4 complete: generated-result review exposes failed fits, read-only projected card outlines, measured errors, failed gates, and worst-frame links. The ten M0 failures still do not publish, but all ten now retain candidates. All 24 local results remain unpublished without independent full-card outlines; nine of ten synthetic cases pass and uniform 10% shrink fails size bias. M5 remains. |
