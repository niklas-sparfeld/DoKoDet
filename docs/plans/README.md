...
| [0045 — Card event review workflow improvements](3-in-progress/0045-Card_Event_Review_Workflow_Improvements.md) | 0039 M0 through M4 complete | M0 complete: wide video-first recording layout, accessible recording-details dialog, German-keyboard-safe marker and frame controls, destination-row selection, and explicit frame-nudge blockers; M1 complete: recording-owned CardEvent review resources with stable IDs, single-draft enforcement, completion lineage, immutable artifacts, and generated API types; M2 complete: unified stable event collection with idempotent commands, durable proposal lineage, and reviewed-only completion projection; M3 complete: verified source-context caching, lean event-command responses, and repeatable local latency measurements; M4 complete: recording-level review collection, stable direct-load review pages, read-only completed history, and responsive route coverage. |
