...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0070 — Synthetic visible-region training data](3-in-progress/0070-Synthetic_Visible_Region_Training_Data.md) | 0068 frozen training inputs plus explicitly reviewed training-only card sources; face-down cutout required before face-down or mixed-side rendering | M0–M3 complete with a declared gap: the setup-matched renderer produced one inspected scene per supported bucket and a train-only merged COCO view. The 0646 sealed-test links are excluded, validation and sealed-test bytes remain unchanged, and 13 near-duplicate pairs are flagged for the fixed comparison. Face-down and mixed-side buckets remain omitted until a reviewed face-down cutout is available. |
