...
| [0076 — Frame-complete automatic calibration candidates](3-in-progress/0076-Frame_Complete_Calibration_Candidates.md) | Completed 0075 robust automatic table-plane calibration | M0 complete: projected full-card outlines outside the source frame are excluded before final fit metrics. The frozen comparison shows lower mean fit and held-out boundary errors, lower spatial-coverage pass count, and no real publications without independent full-card outlines. M1 for occlusion selection is not started and requires user confirmation. |
