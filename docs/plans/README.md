...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0037 — Local visible-card detector end-to-end PoC](3-in-progress/0037-Local_Visible_Card_Detector_PoC.md) | Completed data, analyzer, and backend foundations | Fine-tune one RF-DETR Large detector from Gemini proposals and prove the local provider in the normal configurable backend path. M2 fixture path complete; real detector run and backend selection pending inputs. |
