...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0072 — Pose-based visible-card review](3-in-progress/0072-Pose_Based_Visible_Card_Review.md) | 0048 and 0049 complete | M0–M1 complete: shared calibrated card-plane geometry and versioned scene contracts are frozen; M1 mines deterministic isolated-card candidates, validates a recording-global fit, and stores immutable calibration revisions. M2–M5 not started. |
...
| [0071 — Coarse-to-fine visible-card detection](3-in-progress/0071-Coarse_to_Fine_Visible_Card_Detection.md) | 0048 pipeline data and execution, 0049 recording pipeline review, and the 0068 reviewed RF-DETR training corpus, checkpoint, provider, and evaluation boundaries | M0–M2 complete: the development cascade and deterministic full-frame `card_cluster` train/validation COCO view are implemented with source-linked lineage, exclusion receipts, coverage/scale reports, and reproducible materialization. M3–M6 not started. |
