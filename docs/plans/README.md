...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0070 — Synthetic visible-region training data](3-in-progress/0070-Synthetic_Visible_Region_Training_Data.md) | 0068 frozen training inputs plus explicitly reviewed training-only card sources; face-down cutout required before face-down or mixed-side rendering | M0–M3 complete with a declared gap; M4 preflight is ready; M5 keeps a 38-scene conservative baseline plus a 546-image, 50.42% synthetic ratio-comparison pool from 182 train candidates, using three geometry frames per table setup and up to four cards. Validation and sealed-test recordings are discovery-only. Full RF-DETR training remains in progress for the baseline. Face-down and mixed-side buckets remain omitted until a reviewed face-down cutout is available. |
