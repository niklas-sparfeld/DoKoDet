...
| Epic | Depends on | Outcome |
| --- | --- | --- |
| [0063 — Current CardEventNet training campaign](3-in-progress/0063-Current_CardEventNet_Training_Campaign.md) | 0020, 0028, 0048, and 0049 complete; 0062 M0 and M1 complete | M0–M6 complete. The first campaign retained a loadable checkpoint but failed its gates. M6 records 1,778 point events and 368 interval events, marks 38 recordings eligible, and publishes the five-recording old-phone exclusion as repository-wide legacy-device diagnostic only. Later milestones freeze 27 train, six validation, and five test recordings, isolate the interval-data change, and make all long commands operator-run. |
...
1. **Continue 0063 with M7.** Freeze and materialize the interval-aware dataset after recalculating
   group-safe train, validation, and test assignments. Keep the five old-phone recordings out of
   future datasets, negative mining, gates, and promotion decisions. The operator runs all long
   training, test, export, and optional diagnostic commands; agents only prepare commands and
