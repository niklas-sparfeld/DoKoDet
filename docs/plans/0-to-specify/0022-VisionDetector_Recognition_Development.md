...
- **Summary:** Find a real event-level card recognizer after data and training foundations exist
- **Status:** To Specify
- **Depends on:** Plans 0020 and 0021, with enough reviewed real events for held-out evaluation
...
- reviewed real event count by card, physical copy, deck, session, and device class;
- complete, incomplete, false-event, and not-visible package counts;
...
Gate: continue when card identity is recoverable from the pixels that event evidence captures.
...
Include false CardEventNet events and older cards already on the table.

Gate: continue when localization recall supports useful event-level recognition.
...
if perspective failures dominate. Evaluate each addition against the same held-out events.
...
- event top-1 and top-k accuracy;
- coverage and selective accuracy;
- false-event rejection;
...
- a selected architecture that beats documented event-level baselines;
- evaluation on unseen session groups, including false and incomplete events;
