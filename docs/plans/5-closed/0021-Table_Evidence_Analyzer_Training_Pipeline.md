...
Progress (2026-08-28): M6 is complete. Added explicit CPU, MPS, and CUDA device selection with
availability errors and no implicit fallback. Run metadata records the selected device and
requested precision; the CPU path remains dependency-free. Verification: `mise exec -- uv run
pytest` (21 passed), `mise exec -- uv run ruff check src tests`, and `mise exec -- uv run ruff
format --check src tests`; accelerator hardware was not available, so no real MPS or CUDA smoke
run was performed.
