...
This index records all 43 local raw videos with matching annotation files. It is the V1 metadata
baseline, not final annotation review. The five staged kitchen recordings are indexed intake and
are explicit unassigned records in the development split.
...
- 27 uppercase `.MOV` files contain recent staged trick sequences recorded with the target
  iPhone 14;
- five uppercase `.MOV` files contain staged kitchen activity recorded with the target iPhone 14.

The manifest treats independent staged capture groups as separate sessions and table setups. The
five kitchen recordings share one staged session and table setup.

## Split use

[`full-frame-development.yaml`](../../card_event_net/data/splits/full-frame-development.yaml)
is session-, game-, and table-setup-isolated. Training contains the January 24 real-game round and
27 staged recordings. Validation contains the four January 28 real-game rounds and six staged
recordings.
The staged validation data covers both older recording dates and both iPhone 14 recording dates.
`IMG_0669`, `IMG_0670`, `IMG_0671`, `IMG_0673`, and `IMG_0674` remain unassigned while the staged
kitchen activity is reviewed for future use.
