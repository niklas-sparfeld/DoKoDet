    ...
    *,
    ...
    for event_index, event in enumerate(events.events, start=1):
        ...
        if visible_outcome is None:
            raise ObservationAssemblyError(
                f"event has no visible-card outcome: {event.event_id}"
            )
        ...
        for candidate in visible_outcome.candidates:
            identity = next(
                (
                    item
                    for item in visual_identities.outcomes
                    if item.card_id == candidate.card_id
                ),
                None,
            )
            if identity is None:
                identity_statuses[candidate.card_id] = "missing"
                continue
            identity_statuses[candidate.card_id] = identity.status
            converted = _identity_candidates(identity.candidates)
            if identity.status != "classified" or not converted:
                continue
            cards.append(
                ObservedCard(
                    observed_card_id=candidate.card_id,
                    identity_candidates=converted,
                )
            )
