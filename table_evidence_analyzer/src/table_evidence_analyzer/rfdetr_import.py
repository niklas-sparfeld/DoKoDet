"""Load RF-DETR without importing the optional PyAV video backend."""
    ...
    def find_spec(self, fullname: str, path: object | None = None, target: object = None) -> None:
        if fullname == "av" or fullname.startswith("av."):
            raise ModuleNotFoundError(
                "PyAV is not needed by RF-DETR and is blocked during its import."
            )
        return None
...
def block_pyav_import() -> Iterator[None]:
    """Prevent optional PyAV from loading while RF-DETR imports torchvision."""
